...
execute if entity @s[tag=!rallous.session.done] unless entity @e[tag=rallous.session.camp,limit=1] run tellraw @s {"text":"The night is spent. One village held, or one fight survived. That is enough.","color":"gold"}
execute if entity @s[tag=rallous.session.done] run tellraw @s {"text":"This night is already spent.","color":"gray"}
