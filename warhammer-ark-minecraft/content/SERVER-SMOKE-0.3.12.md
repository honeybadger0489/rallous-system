# SERVER-SMOKE-0.3.12 — Forge dedicated boot

**Done: YES.** `Done (2.306s)!` and **no** `ModLoadingException`.

Date: 2026-09-01. Runtime: Java 17.0.20, Minecraft 1.20.1, Forge **47.4.10**. JVM `-Xms4G -Xmx6G`. Reused `/tmp/rallous-smoke-039` from the 0.3.11 smoke (existing world). ModernFix: dedicated server took 20.61s.

## Zip / pin

| Check | Result |
| --- | --- |
| Zip | `dist/rallous-warhammer-fantasy-0.3.12.zip` (24 810 888 bytes) |
| Manifest | `minecraft 1.20.1`, `forge-47.4.10`, version `0.3.12`, **76** CF files |
| Server dir | `/tmp/rallous-smoke-039` (Forge 47.4.10 installer already present) |
| Refresh | Overwrote all 14 `rallous_*` / `rallous-old-world` / `rallous-recruits-bridge` jars from the 0.3.12 zip. CF libs unchanged. |

Changed vs 0.3.11 install: `rallous-old-world-1.0.0.jar` 31 106 → 22 077 (Karl / Kislev leftover letters already cut in 0.3.12), `rallous_winds-1.0.0.jar` 16 773 → 16 802. Other Rallous jars same size.

## Dedicated park (same as 0.3.11)
...
```
[Server thread/INFO] [net.minecraft.server.dedicated.DedicatedServer/]: Done (2.306s)! For help, type "help"
[Server thread/WARN] [ModernFix/]: Dedicated server took 20.61 seconds to load
```

`ModLoadingException` count: **0**. No new missing CurseForge / FML deps.

Full log: [`SERVER-SMOKE-0.3.12.latest.log`](SERVER-SMOKE-0.3.12.latest.log)  
...
## Remaining non-function leftovers (same class as 0.3.11)
...
| New zip | **none at smoke time** (0.3.12 was the zip under test) |
