...
Date: 2026-09-01. Runtime: Java 17.0.20, Minecraft 1.20.1, Forge **47.4.10**. JVM `-Xms4G -Xmx6G`. Reused `/tmp/rallous-smoke-039` from the 0.3.12 smoke (existing world). ModernFix: dedicated server took 18.027s.

## Zip / pin

| Check | Result |
| --- | --- |
| Zip | `dist/rallous-warhammer-fantasy-0.3.13.zip` (24 803 257 bytes at integrate) |
...
| Refresh | Overwrote all 14 `rallous_*` / `rallous-old-world` / `rallous-recruits-bridge` jars from the 0.3.13 zip. CF libs unchanged. Disk sizes matched zip. |

Changed vs 0.3.12 install: `rallous-old-world-1.0.0.jar` 22 077 → 16 296 (CNPC leftover letters, Kislev commissions, unused lord advancements stripped). Other Rallous jars same size.

## Dedicated park (same as 0.3.12)
...
## Remaining non-function leftovers (same class as 0.3.12)
...
| Leftover court chrome in this zip | **stripped** (CNPC letters / Kislev commissions / unused lord advancements) |
