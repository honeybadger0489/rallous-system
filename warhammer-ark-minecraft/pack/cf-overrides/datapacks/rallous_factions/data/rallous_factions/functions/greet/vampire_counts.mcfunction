tellraw @a[distance=..48] [{"text": "<Master Necromancer Helman Ghorst — Vampire Counts> ", "color": "dark_red", "bold": true}, {"text": "Plague-cart scholar of undeath. Prove you can fetch corpses; he will not ride to save the living. Prove yourself. Until then you are a Warp-stranger, not a guest.", "color": "yellow"}]
...
execute if score @s rallous.fac.stance matches 3 run function rallous_factions:raid/vampire_counts
execute if score @s rallous.fac.stance matches 6 run function rallous_factions:raid/vampire_counts
