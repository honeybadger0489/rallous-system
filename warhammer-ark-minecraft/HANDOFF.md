...
**PLAY.md + `pack/curseforge/manifest.json` + `dist/`:** **0.3.11**

File: `warhammer-ark-minecraft/dist/rallous-warhammer-fantasy-0.3.11.zip`

Raw: https://github.com/honeybadger0489/rallous-system/raw/cursor/warhammer-ark-minecraft-d8d1/warhammer-ark-minecraft/dist/rallous-warhammer-fantasy-0.3.11.zip

Older archives (0.2.0–0.3.10) stay in `dist/` for history. **Import 0.3.11 as a new CurseForge profile.** Do not update 0.2.1 / 0.2.2 / 0.3.0–0.3.10.

**0.3.11** is the player zip: 0.3.10 plus director-pass first-hour tellraws, honesty cuts in PLAY/wiki, leftover Recruits/Fossils ID fixes (`recruits:patrol_leader`, `fossil:egg_item_triceratops`), and the rebuilt Recruits bridge (optional Recruits / HostFounder NPE harden). Dedicated-server smoke **passed on 0.3.10**. **0.3.11 is unsmoked.**
...
Sibling folders under `content/datapacks/` compile into LowCodeFML jars under `pack/cf-overrides/mods/`. **0.3.11 ships jars only** (folder copies in `overrides/datapacks/` are dropped so a world copy cannot double-fire `#minecraft:tick`). Enable **folder or jar, not both**.
...
| `rallous-recruits-bridge` | Java Forge jar. Calls Recruits `FactionEvents.createTeam(false, …)` so U / chat say **Reikland** / **Clan Mors**, not Team 2 | Source: `mods/rallous-recruits-bridge/`. API notes: [`content/RECRUITS-API.md`](content/RECRUITS-API.md). In the 0.3.11 zip (optional Recruits / NPE-hardened rebuild). |
...
python3 scripts/integrate-overrides.py --version 0.3.11
```

`integrate-overrides.py` (default `--version 0.3.11`):
...
6. `pack-zip.py --version …` → `dist/rallous-warhammer-fantasy-0.3.11.zip`
...
**0.3.10 dedicated-server smoke (2026-09-01):** **boot YES.** Reused the 0.3.9 Forge 47.4.10 dir, refreshed `rallous_*` jars from `dist/rallous-warhammer-fantasy-0.3.10.zip`. `Done (1.918s)`, no `ModLoadingException`, **0** `Failed to load function`. Report: [`content/SERVER-SMOKE-0.3.10.md`](content/SERVER-SMOKE-0.3.10.md). Log: [`content/SERVER-SMOKE-0.3.10.latest.log`](content/SERVER-SMOKE-0.3.10.latest.log). ETF/Oculus/Embeddium/EMF parked for dedicated only (client mixin).
